...
// Empty string is a valid additional ID so allow empty string but disallow null.
            if (TextUtils.isEmpty(valuesClientId) || null == valuesMetadataUri || null == valuesMetadataAdditionalId) { // We need all these columns to be filled in
                com.bondiola.nicoliniboard.inputmethod.latin.utils.DebugLogUtils.l("Missing parameter for updateClientInfo")
                return
            }
            if (clientId != valuesClientId) { // Mismatch! The client violates the protocol.
                com.bondiola.nicoliniboard.inputmethod.latin.utils.DebugLogUtils.l("Received an updateClientInfo request for ", clientId,
        ...
        fun markEntryAsFinishedDownloadingAndInstalled(db: SQLiteDatabase,
            ...
            when (r.getAsInteger(TYPE_COLUMN)) {
                TYPE_BULK -> {
                    com.bondiola.nicoliniboard.inputmethod.latin.utils.DebugLogUtils.l("Ended processing a wordlist")
...
// the phone is suddenly cut during an update.
                            ...
                            do {
                                com.bondiola.nicoliniboard.inputmethod.latin.utils.DebugLogUtils.l("Setting for removal", c.getString(filenameIndex))
